<?php
/**
 * auth.php — tek yönetici oturumu, kaba kuvvet kilidi ve CSRF koruması
 */
...
function kullanici_bul(string $epostaVeyaAd): ?array {
    ...
    $u = kullanici_oku()[0] ?? null;
    if ($u && (mb_strtolower($u['eposta'] ?? '') === $k || mb_strtolower($u['kadi'] ?? '') === $k)) {
        return $u;
    }
    return null;
}
...
function oturum_gerekli(): array {
    ...
    // Tek yönetici hâlâ var mı? Oturum mührü tutuyor mu?
    $u = kullanici_oku()[0] ?? null;
    if ($u && $u['id'] === $_SESSION['uid']) {
...
/** Panel tek yönetici hesabıyla çalışır; eski çağrılar için uyumluluk katmanı. */
function yonetici_mi(): bool { return !empty($_SESSION['uid']); }
function yonetici_gerekli(): void { oturum_gerekli(); }
