...
function surum_al(string $ad): void {
    // Otomatik sürüm/yedek üretimi sadeleştirilmiş panelde kapalıdır.
}
...
function kullanici_yaz(array $liste) {
    // Panel tasarımı tek yönetici hesabını destekler; ikinci kayıt tutulmaz.
    $liste = array_slice(array_values($liste), 0, 1);
    $ok = json_yaz(VERI_DIZIN . '/users.json', ['kullanicilar' => $liste]);
...
function site_json_uret(): bool {
    ...
    $cikti = [
        ...
        'gorseller'  => $ayar['gorseller'] ?? [],
        'marka'      => ['logo' => 'assets/img/logo.png'],
        'menu'       => $ayar['menu'] ?? [],
