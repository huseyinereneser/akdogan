  ...
  <?php foreach ($sayfalar as $sk => $sayfa): ?>
    <div class="kart">
      ...
      <?php foreach (($sayfa['alanlar'] ?? []) as $ak => $alan): $yol = $sk . '.' . $ak; ?>
        <div class="alan ceviri-alani" data-ceviri-grup="<?= e($yol) ?>">
          <div class="alan-baslik"><label><?= e($alan['ad'] ?? $ak) ?> <span class="ipucu"><?= e($yol) ?></span></label><button type="button" class="btn btn--ceviri" data-ceviri-butonu>TR → EN</button></div>
          ...
          <div class="sekme-govde aktif" data-grup="<?= e($yol) ?>" data-sekme="tr">
            <textarea name="tr[<?= e($yol) ?>]" data-ceviri-kaynak><?= e($alan['tr'] ?? '') ?></textarea>
          </div>
          <div class="sekme-govde" data-grup="<?= e($yol) ?>" data-sekme="en">
            <textarea name="en[<?= e($yol) ?>]" data-ceviri-hedef><?= e($alan['en'] ?? '') ?></textarea>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
